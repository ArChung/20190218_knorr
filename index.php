...
<body>
    ...
    <div id="app" v-cloak>
        <div id="logo"></div>
        <div id="menuBtn" @click='menuOpen=true' class='phoneShow'></div>
        <div id="mainContainer">
            ...
            <div id="mainPage" :class="{hide:pageChannel!='mainPage'}">
                ...
                <div id="fieldContainer">
                    <div class="field">
                        ...
                        <!-- animal -->
                        <div class="tagBtn" @click='openAnimal(index)' @mouseover='stopDemo' @mouseleave='startDemo' v-for="(item,index) in animal.ani_data" :class='[item.className]'>
                            <div class="tag">{{item.name}}</div>
                        </div>

                        <!-- plant -->
                        <div class="tagBtn" @click='openPlant(index)' @mouseover='stopDemo' @mouseleave='startDemo' v-for="(item,index) in plant.data" :class='[item.className]'>
                            <div class="tag">{{item.name}}</div>
                        </div>
                    ...
                    <div class="subTitle ta_c ls-1">
                        ...
                        <div class="gBtn mx_a mt_m aniBtn gaBtn" data-ga='quiz_start' @click='reQA'>開始作答</div>
                    </div>
                </div>
                <div id="infoContainer">
                    <div class="innerContainer">
                        ...
                        <div class="productWrap mt_bb mx_a">
                            <!-- elments -->
                            <div class="gBBtn" v-for="(item,index) in elements.data" @click='openElement(index)' :key='index'>
                                <div class="txtWrap">
            ...
            <!-- PAGE:  PRODUCT -->
            <div id="product" :class="{hide:pageChannel!='product'}">
                <div class="container">

                    <h1 class='ta_c ls-1 fwb'>自然原點<br>也是美味的起點</h1>
                    <div class="video mt_b">
                    ...
                    <swiper ref="product_Swiper" id='product_Swiper' :options="product_SwiperOption" class='mt_b product_Swiper mx_a' @transition-end="playSwiperAni" @transition-start='wannaBuy=false;'>
                    ...
                    <div class="df jcsa fww productTable mx_a mt_m phoneHide">
                        <div class="pro scaleBtn" v-for='(data,index) in products.products' :key='index' @click='product_Swiper.slideTo(index);pageScrollAni("#product_Swiper")'>
            ...
            <!-- PAGE:  RULE -->
            <div id="rule" :class="{hide:pageChannel!='rule'}">
                <div class="container f18">
                    <h1 class='ta_c ls-1 fwb'>活動辦法</h1>
                    ...
                    <transition name='fadeInLeft'>
                        <div class="subContainer" v-if='ruleSwitch.s1'>
                            <p>
                                活動日期: <br>2019/03/15(五)–2019/04/26(五)止
                            </p>
                            <p>
                                玩法：
                            </p>
                        ...
                        <div class="subContainer" v-if='ruleSwitch.s2'>
                            <ul class="numList">
                                ...
                                <li class='mt_m'>康寶濃湯自然原味玉米系列二袋」週週抽將於03/25、04/01、04/08、04/15、
                                    04/22、04/29 中午12:00各別抽出三名前一週的得獎者，中獎名單將於 03/27、04/03、04/10、04/17中午12:00依序公布於活動網站。</li>
                    ...
                    <transition name='fadeInLeft'>
                        <div class="subContainer" v-if='ruleSwitch.s3'>
                            <p>活動日期: <br>2019/4/15(五)–2019/04/26(五)止</p>

                            <p>
                                玩法： <br>參加「康寶 no.1料理點子王」粉絲團「是誰住在友善農場裡？」相關活動貼文，依貼文規定回答問題，將抽出六名答對者，贈送「康寶2人份熱賣口味濃湯組合一份」。
                            </p>
                        </div>
                    </transition>
                    ...
                    <!-- 蒐集個人資料聲明 -->
                    <div class="gTxt f28 mt_b switcher" :class='{ open : ruleSwitch.s6 }' @click='ruleSwitch.s6=!ruleSwitch.s6'>蒐集個人資料聲明</div>
                    <transition name='fadeInLeft'>
            ...
            <div class="popUp" v-if='popChannel!=""'>
                <div class="innerPopup">
                    ...
                    <div class="qAWrap" v-if='popChannel=="qa"'>
                        ...
                        <transition name='qa'>
                            <div class="result container" v-if='qa.state==="result"'>
                                ...
                                <h2 class="ta_c darkredTxt">{{qa.result[qa.score].title}}
                                </h2>
                                <h3 class="mt_s" v-html="qa.result[qa.score].info"></h3>
                                <div class="btns mt_m">
                                    <div class="gBtn mx_a" @click='retry' v-if='qa.score < 5'>再試一次</div>
                        ...
                        <div class="answer container" v-if='qa.state==="answer"'>
                            ...
                            <div class="btns mt_s">
                                <div class="gBtn mx_a" @click='nextQuestion' v-if="qa.index<4">下一題</div>
                                <div class="gBtn mx_a" @click='nextQuestion' v-else>看結果</div>
                            </div>
                            <h3 class="gTxt ta_c fwb">{{ qa.index+1 + " / 5"}}</h3>
                        </div>
                        <!-- </transition> -->
                    ...
                    <div class="plantWrap container" v-if='popChannel=="plant"'>
                        ...
                        <div class="gBtn mt_m mx_a arrBtn" @click='plantsPopupSeeMore(plant.index)'>看更多</div>
                    </div>

                    <!-- animal pop -->
                    <div class="animalWrap container" v-if='popChannel=="animal"'>
                        <div class="cloz" @click='popChannel=""'></div>
                        <div class="pic" :class='animal.ani_data[animal.index].className'>
                        </div>
                        <div class=" ta_c">
                            ...
                            <form >
                                ...
                                <div class="mt_m">
                                    <label>
                                        ...
                                        <div class="txt f12">我同意接收聯合利華及其合作夥伴的市場推廣資訊。</div>
                                    </label>
                                    <label>
                                        <input class="styled-checkbox small" type="checkbox" v-model='userData.agreeRule'>
                                        <div class="txt f12">我已詳讀<a href="?page=rule" target='_blank' class='underline'>活動辦法與蒐集個人資料聲明</a>。</div>
                                    </label>
                                </div>
        ...
        <!-- menu -->
        <div id="menu" :class='{phoneHide:!menuOpen}'>
            <div class="cloz" @click='menuOpen=false'></div>
            <div class="inner f32  fwb gTxt">
                <div class="mBtn scaleBtn gaBtn" data-ga='menu_index' @click='onMenu("map")'>一起逛農場</div>
                <div class="mBtn scaleBtn gaBtn" data-ga='menu_quiz' @click='onMenu("qa")'>阿康伯出考題</div>
                <div class="mBtn scaleBtn gaBtn" data-ga='menu_info' @click='onMenu("plant")'>友善農場的秘密</div>
                <div class="mBtn scaleBtn gaBtn" data-ga='menu_product' @click='onMenu("product")'>美味販賣部</div>
                <div class="mBtn scaleBtn gaBtn" data-ga='menu_rule' @click='onMenu("rule")'>活動辦法</div>
                <div class="mBtn scaleBtn gaBtn" data-ga='menu_award' @click='onMenu("winner")'>中獎名單</div>
